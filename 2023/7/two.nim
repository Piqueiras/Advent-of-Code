...
proc rankWithoutJoker(card:string): Hand =
    for letter in poker:
...
proc rank(card:string): Hand =
    if 'J' notin card:
        return rankWithoutJoker(card)
    var results = newSeq[Hand]()
    for letter in poker[0..^2]:
        results.add(rankWithoutJoker(card.replace('J',letter)))
    return results.min #Its supposed to be max but for some reason
